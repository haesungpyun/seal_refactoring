...
class Constructor(object):
    ...
    def construct_class(
        ...
        **kwargs:Any
        ...
        if config is None:
            return 
        module, constructor, args = self.import_class(config, object_name)
    ...
    def init_and_return_attr(
        ...
        **kwargs: Any
        ...
        for key in checked_params.keys():
            try:
                kwargs.pop(key)
            except:
                continue
        ...
        if constructor is None:
            final_class = module(**checked_params, **kwargs)
        else:
            final_class = getattr(module, constructor)(**checked_params, **kwargs)
    ...
    def construct_args(
        ...
        **kwargs:Any      
        ...
        for param_name, param in parameters.items():
            ...
            pop_param = kwargs.pop(param_name, None)
            if pop_param is not None :
                checked_param[param_name] = pop_param
                continue
            ...
            if "module_dot_path" not in str(config_param):
                ...
                checked_param[param_name] = self.recursive_construct(config_param, param_name, **kwargs)
        ...
        **kwargs
        ...
        if type(config_param) == dict:
            ...
            if "module_dot_path" in config_param.keys() or 'object_name' in config_param.keys():
                return self.construct_class(config_param, param_name, **kwargs)
            else:
                for key, value_params in config_param.items():
                    tmp[key] = self.recursive_construct(value_params, param_name+'.'+key, **kwargs)
                return tmp
            ...
            for i, value_params in enumerate(config_param):
                tmp.append(self.recursive_construct(value_params, param_name+'.'+str(i), **kwargs))
